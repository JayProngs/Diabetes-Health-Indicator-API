...
# Define the recipe using the predictors from the best model
...
final_rf_model <- rand_forest(
  mtry = 4,   # Replace with your best mtry value
  trees = 1000
) %>%
...
function(
  ...
  input_data <- tibble(
    ...
    GenHlth = factor(GenHlth, levels = c("Excellent", "Very Good", "Good", "Fair", "Poor"), ordered = TRUE),
    Age = factor(Age, levels = c("18-24", "25-29", "30-34", "35-39", "40-44", "45-49", "50-54",
                                 "55-59", "60-64", "65-69", "70-74", "75-79", "80 or older"), ordered = TRUE)
  )
  ...
  # Predict probability
  prediction <- predict(final_rf_fit, baked_data, type = "prob")
  
  # Return the probability of 'Yes' (having diabetes)
...
function() {
  list(
    name = "Jay",
    github_pages_url = "https://github.com/JayProngs/Diabetes-Health-Indicator-API/EDA.html"
...
function() {
  ...
  # Print the plot to output it as a PNG
  print(plot)
}
